...
type FileController struct {
	UploadFileCloudinary (*gin.Context)
}
...
func NewFileController(cloud storage.Cloudinary) *FileControllerImpl {
	return &FileControllerImpl{
		cloudinary: cloud,
	}
}

func (c *FileControllerImpl) UploadFileCloudinary(ctx *gin.Context) {
	...
	for _, file := range files {
		...
		if createTextFile {
			...
			request := uploader.UploadParams{
				Folder:                   path,
				UseFilenameAsDisplayName: &useFileName,
				UseFilename:              &useFileName,
				UniqueFilename:           &UniqueFilename,
			}
			c.cloudinary.UpdateAsset(file, request)
		}
		if saveToDatabase {
			fileObj := &model.File{
